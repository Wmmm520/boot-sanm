...
import com.sanm.core.process.RedisDelayQueueProcess;
import com.sanm.utils.ExceptionUtil;
import com.sanm.exception.RedisDelayQueueException;
import com.sanm.redis.DefaultRedisLuaHandler;
import com.sanm.redis.RedisHandler;
import com.sanm.threads.Move2ReadyThread;
import com.sanm.threads.RetryOutTimesThread;
import com.sanm.threads.ShutdownThread;
import com.sanm.utils.BaseUtil;
import com.sanm.utils.NetUtil;
import com.sanm.utils.NextTimeHolder;
import com.sanm.utils.TimeoutUtil;
...
public class RedisDelayQueueContext {
     ...
     * 保存所有的Topic的回调接口
     **/
    private static ConcurrentHashMap<String, AbstractTitleRegister> topicRegisterHolder = new ConcurrentHashMap<>();
    ...
    final protected void initCtx(RedisTemplate<Object, Object> redisTemplate, String projectName, String ipInRedisServer) {
        if (projectName.contains(":") || projectName.contains("{") || projectName.contains("}")) {
            throw new RedisDelayQueueException("projectName 不能包含特殊字符 : } { ");
        }
        this.redisHandler = new DefaultRedisLuaHandler(redisTemplate);
        this.redisDelayQueue = new RedisDelayQueueProcess(redisHandler, topicRegisterHolder, DelayQ_ASYNC);
    ...
    private void init() {
        //启动监听Bucket线程
        Move2ReadyThread.getInstance().runMove2ReadyThread(redisHandler);
        //支持BLPOP吗
        checkBlpop();
        //5秒后启动Topic的监听线程池
    ...
    private void runTopicsThreads() {
        ...
        for (Map.Entry<String, AbstractTitleRegister> entry : topicRegisterHolder.entrySet()) {
            AbstractTitleRegister register = entry.getValue();
            runTopicThreads(register);
        }
    }

    private void runTopicThreads(AbstractTitleRegister register) {
        ...
        Thread a = new Thread(() -> {
            ...
            while (!topicThreadStop) {
                try {
                    ...
                    if (maxGet <= 1 ||
                            (null == (topicIds = redisHandler.lRangeAndLTrim(register.getTopic(), maxGet))
                                    || topicIds.size() == 0)) {
                        ...
                        if (canUseBlpop) {
                            ...
                            String topicId = redisHandler.BLPopKey(register.getTopic());
                    ...
                    if (topicIds != null && topicIds.size() > 0) {
                        ...
                        for (int i = 0; i < topicIds.size(); i++) {
                            ...
                            register.getTOPIC_THREADS().execute(() -> {
                                ...
                                Base args = null;
                                try {
                                    args = redisHandler.getTask(topicId + "");
                                    if (args != null) {
                                        try {
                                            checkTimeoutExectue(register.getMethodTimeout(), register, args);
                                            if (args.getRetryCount() > 0) {
                                                logger.info("重试延迟任务第{}次重试消费成功,topicId:{},Args:{} ", args.getRetryCount(), BaseUtil.getTitleId(register.getTopic(), args.getId()), args.toString());
                                            } else {
                                                logger.info("延迟任务消费成功,topicId:{},Args:{} ", BaseUtil.getTitleId(register.getTopic(), args.getId()), args.toString());
                                            }
                                        } catch (Exception e) {
                                            if (args.getRetryCount() > 0) {
                                                logger.error("重试任务第{}次重试败,执行回调接口出错:topicId:{},Args:{},Err:{}", args.getRetryCount(), BaseUtil.getTitleId(register.getTopic(), args.getId()), args.toString(), ExceptionUtil.getStackTrace(e));
                                            } else {
                                                logger.error("延迟任务消费失败,执行回调接口出错:topicId:{},Args:{},Err:{}", BaseUtil.getTitleId(register.getTopic(), args.getId()), args.toString(), ExceptionUtil.getStackTrace(e));
                                            }
                                            isfail = true;
                                            // 重试流程
                                            RetryOutTimesThread.getInstance().callBackExceptionTryRetry(register, args, redisHandler);
                                        }
                                    }
                                } catch (Exception e) {
                                    ...
                                    if (args != null &&
                                        ...
                                        redisHandler.deleteTask(register.getTopic(), args.getId());
                                    }
                                    semaphore.release();
                                }
                            });
    ...
    private void checkTimeoutExectue(final long timeout, AbstractTitleRegister register, Base args) throws InterruptedException, ExecutionException, TimeoutException {
     ...
     */
    private void againRightPush(String topic, Base args) {
        if (args.getRetryCount() > 2) {
            logger.error("未被消费任务topicId:{},重入了3次仍旧失败", BaseUtil.getTitleId(topic, args.getId()));
        } else {
            RPUSH_NO_EXEC_JOB.execute(() -> {
                //获取JOb的方法连接失败了 异步重新放入;
                logger.warn("未被消费任务topicId:{} 重新放入待消费队列", BaseUtil.getTitleId(topic, args.getId()));
                args.setRetryCount(args.getRetryCount() + 1);
                redisHandler.retryTask(topic, args.getId(), args);
            });
        }
    ...
    final public static void addTopic(String topic, AbstractTitleRegister register) {
        if (null != topicRegisterHolder.get(topic)) {
            throw new RedisDelayQueueException("Topic 注册重复,请保证Topic唯一");
        }
    ...
    private void killThisMachineAllRedisBlpopClients(RedisHandler redisHandler) {
        String ip = ipInRedisServer;
        if (StringUtils.isEmpty(ip)) {
            ip = NetUtil.getLocalHostLANAddress();
        }
        logger.info("ipInRedisServer;{}", ip);
        List<RedisClientInfo> list = redisHandler.getThisMachineAllBlPopClientList();
